    ...
    constructor(props) {
      ...
      render() {
        ...
        <FlatList
          ...
          renderItem={({item}) =>
          <View >
            <ProductCardComponent  sellerName={item.SellerName} Status={item.Status} BuyerID={item.BuyerID} thumbnail={item.Thumbnail} pickupAddress={item.AddressArray} owner={item.Owner} id ={item.key} title = {item.Name} description = {item.Description} price = {item.Price}  pictures = {item.Pictures}  category = {item.Category}/>
          </View>
          }
        />
        </ScrollView>
